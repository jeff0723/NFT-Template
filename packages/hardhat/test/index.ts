...
describe("Template NFT", function () {
  it("Simple flow", async function () {
    ...
    const contract = TemplateNFT__factory.connect(
      deployment.address,
      owner
    );
    ...
    assert((await owner.provider.getBalance(contract.address)).eq(totalSupply.mul(STAGE_1_PRICE)));
    ...
    await expect(contract.connect(user1).whitelistMint(voucher1, signatureForUser0, 2, { value: STAGE_1_PRICE.mul(2) }))
    .to.be.revertedWith("invalid or unauthorized");

    const signatureForUser1 = await owner._signTypedData(domainData, VOUCHER_TYPE, voucher1);
    tx = await contract.connect(user1).whitelistMint(voucher1, signatureForUser1, 4, { value: STAGE_1_PRICE.mul(4) });
    await tx.wait();
    ...
    tx = await contract.connect(user1).whitelistMint(voucher1, "0x00", 2, { value: STAGE_1_PRICE.mul(2)});
    await tx.wait();
    ...
    await expect(contract.connect(user1).whitelistMint(voucher1, "0x00", 4, { value: STAGE_1_PRICE.mul(4) }))
    ...
    await expect(contract.connect(user1).whitelistMint(voucher1, signatureForUser1, 2, { value: STAGE_1_PRICE.mul(2) }))
    .to.be.revertedWith("Not enough remain");

    await expect(contract.connect(user0).reserve(user0.address, 9)).to.be.revertedWith("Ownable: caller is not the owner");
    ...
    assert((await owner.provider.getBalance(contract.address)).eq(totalSupply.sub(2).mul(STAGE_1_PRICE)));

    await network.provider.send("evm_setNextBlockTimestamp", [STAGE_1_END+1]);
    await network.provider.send("evm_mine");

    await expect(contract.connect(user1).whitelistMint(voucher1, "0x00", 1, { value: STAGE_1_PRICE }))
    .to.be.revertedWith("Sale already ended");

    await expect(contract.connect(user2).publicMint(2, { value: STAGE_2_PRICE.mul(2) }))
    ...
    await expect(contract.nextStage(stage2Info))
    .to.be.revertedWith("Public is final stage");
    stage2Info.stageId = 2;
    stage2Info.maxSupply = 20000;
    await expect(contract.nextStage(stage2Info))
    .to.be.revertedWith("Set exceed max supply");

    stage2Info.maxSupply = 6999;
    ...
    tx = await contract.connect(user1).publicMint(1, { value: STAGE_2_PRICE.mul(1) })
    await tx.wait();
    assert((await contract.balanceOf(user1.address)).eq(9));

    tx = await contract.reserve(user3.address, 7);
    await tx.wait();
    ...
    if (receipt.status === 1) {
      ...
      assert(ownerBalanceBefore.add(totalEarn).sub(gasFee).eq(ownerBalanceAfter));
    }
